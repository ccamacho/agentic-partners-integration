...
async def _ensure_email_mapping(
    ...
    stmt = (
        ...
        .where(
            ...
            UserIntegrationMapping.integration_type == IntegrationType.WEB,
        )
        .limit(1)
    )
    ...
    if not existing_mapping:
        # Create a mapping entry for the email (using email as integration_user_id for WEB type)
        insert_stmt = insert(UserIntegrationMapping).values(
            user_id=canonical_user_id,
            user_email=email_address,
            integration_type=IntegrationType.WEB,
            integration_user_id=email_address,
...
async def get_or_create_canonical_user(email_address: str, db: AsyncSession) -> str:
    ...
    if existing_mapping:
        ...
        # Ensure WEB mapping exists (the existing mapping might be for a different integration type)
    ...
    if existing_user:
        # Ensure WEB mapping exists even if user already exists
    ...
    try:
        ...
        # Ensure WEB mapping exists for the new user
